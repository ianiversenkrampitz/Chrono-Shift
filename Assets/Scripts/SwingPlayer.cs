...
public class SwingPlayer : MonoBehaviour
{
    ...
    public bool jumpedOutOfSwing = false;
    ...
    protected virtual void FixedUpdate()
    {
        ...
        // player falls in air
        Gravity();
        // slow down when not moving, swinging, or dashing
        SlowDown();
    ...
    // when player is not moving, dashing, or swinging, rapidly slow down
    private void SlowDown()
    {
        if (!Moving() && !dashing && !swinging && rigidBodyRef.velocity != Vector3.zero)
        {
    ...
    private void Jump()
    {
        ...
        // player jumped out of swing so tell swing script they're jumping
        else if (jumpInput && swinging)
        {
            jumpedOutOfSwing = true;
            swinging = false;
        }
